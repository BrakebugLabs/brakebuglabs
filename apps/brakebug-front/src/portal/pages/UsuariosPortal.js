...
import { FaUserPlus, FaEdit, FaTimes, FaCheck, FaArrowLeft} from 'react-icons/fa';
...
const UsuariosPortal = () => {
  ...
  const fetchUsuarios = async () => {
    ...
    setError(null);
    try {
      const response = await api.get('/portal/admin/usuarios/');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        ...
        await api.put(`/portal/admin/usuarios/${selectedUser.id}`, updateData);
        setSubmitSuccess('Usuário atualizado com sucesso!');
      } else {
        // Criar novo usuário
        await api.post('/portal/admin/usuarios/', {
          nome: formData.nome,
          email: formData.email,
  ...
  const handleDeactivateUser = async () => {
    ...
    try {
      await api.delete(`/portal/admin/usuarios/${userToDeactivate.id}`);
      fetchUsuarios();
  ...
  const handleAdminAuthCancel = () => {
    // Redirecionar para o Portal se o usuário cancelar a autenticação
    navigate('/portal');
  };
  ...
  if (loading && usuarios.length === 0) {
    return (
      <Container fluid className="text-center mt-5">
        <Button variant="outline-secondary" onClick={goToPortal} className="mb-3">
  ...
  return (
    <Container fluid>
      <Row className="mb-3 align-items-center" style={{ marginTop: '4rem'}}>
        <Col>
          <h1 className="h3">Gerenciamento de Usuários</h1>
        </Col>

        <Col xs="auto" className="d-flex gap-2">
          <Button variant="outline-secondary" onClick={goToPortal} className="d-flex align-items-center">
